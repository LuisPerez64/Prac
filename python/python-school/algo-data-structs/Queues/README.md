# Implementation of the Circular Queue data structures
    A FIFO(First In First Out) data structure. The element that was placed within the QUeue initially is the one that is popped off first. Done with the use of two "Pointers" that hold the location of the first element within the Queue that was placed in, and the location of the last element that was brought into it. 
