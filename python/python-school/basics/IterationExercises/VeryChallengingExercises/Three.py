...
#Takes a hex string 0x..., and makes it back to decimal
def myHexToDec(inputHex):
    inpHex=str(inputHex).split('x')[-1][::-1]
    out=0
    multMe=0
    for i in inpHex:
        val=next(ind for ind,inp in enumerate(hexList) if inp == i)
        out+=(val*16**multMe)
        multMe+=1
    return out
...
My hex implementation: {4}
Back to original from hex: {5}
'''.format(inputInt, bin(inputInt), myBin(inputInt),hex(inputInt), myHex(myBin(inputInt)), myHexToDec(myHex(myBin(inputInt))) )) 
