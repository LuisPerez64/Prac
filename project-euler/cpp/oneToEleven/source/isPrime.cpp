...
/*
  ...
  SideNote::
   ...
   above it.
  SideNote 2:
   Just in case I for some reason forget... Which is possible
   X = 6 = sqrt(6*6) This is holding for the x*x below to remove sqrt(checkMe)
*/
bool isPrime(long int checkMe, std::vector<long int> Primes) {
  ...
  while (result) {
    if(primesIterator == (Primes.end() - 1) or 
       (*primesIterator) * (*primesIterator) > checkMe)
